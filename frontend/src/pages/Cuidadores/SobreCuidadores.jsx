...
const SobreCuidadores = ({name, about, qualifications, experiences}) => {
  return <div>
    <div>
        <h3 className="text-[20px] leading-[30px] text-headingColor font-semibold flex items-center gap-2">
            Sobre
            <span className="text-yellowColor font-bold text-[24px] leading-9">{name}</span>
            </h3>
        ...
        <ul className="pt-4 md:p-5">
            {qualifications?.map((item, index)=>
                <li key={index} className="flex flex-col sm:flex-row sm:justify-between sm:items-end md:gap-5 mb-[30px]">
                    <div>
                        <span className="text-yellowColor text-[15-px] leading-6 font-semibold">{formateDate(item.startingDate)} - {formateDate(item.endingDate)}</span> 
        ...
        <h3 className="text-[20px] leading-[30px] text-headingColor font-semibold">
                ...
                <ul className="grid sm:grid-cols-2 gap-[30px] pt-4 md:p-5">
                    ...
                    {experiences?.map((item,index) => <li key={index} className="p-4 rounded bg-[#fff9ea]">
                        ...
                        <p className="text-[14px] leading-5 font-medium text-textColor">{item.company}</p>
                    </li>
                 )}
                </ul>
        </div>
